...
t_area				*mem_area_init(t_mcfg *dat, size_t psize, size_t allocsize, void *expected_addr)
{
	...
	{
		i = allocsize;
		mapsize = (dat->psize * FTMALLOC_FACTOR) + sizeof(t_area) + sizeof(t_blk) + allocsize;
	}
	else
	{
	...
	start = mmap(expected_addr, mapsize, dat->opts.prot, dat->opts.flags, dat->opts.fd, dat->opts.offset);
